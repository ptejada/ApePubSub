 ...
 */
function randomString(a,c){for(var e=c||"0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz",d=a||32,b="",g=0;g<d;g++)var j=Math.floor(Math.random()*e.length),b=b+e.substring(j,j+1);return b}
...
function APS(a,c,e){this.option={poll:25E3,debug:!1,session:!0,connectionArgs:{},server:a,transport:["ws","lp"],secure:!1,eventPush:!1,addFrequency:!0,autoUpdate:!0};this.identifier="APS";this.version="1.5.7";this.state=0;this._events={};this.chl=0;this.user={};this.pipes={};this.channels={};this.eQueue={};if(c)this.on(c);if(e)for(var d in e)this.option[d]=e[d];"Microsoft Internet Explorer"==navigator.appName&&(this.log="undefined"==typeof window.console?function(){}:function(){if(!1!=this.option.debug){var b=
Array.prototype.slice.call(arguments);b.unshift("["+this.identifier+"]");window.console.log(b.join().replace(",",""))}});this.session._client=this;return this}
APS.prototype.connect=function(a){var c=this.option.server,e={onmessage:this.onMessage.bind(this),onerror:function(b){this.trigger("dead",[b]);this.transport.close()}.bind(this)};if(1==this.state)return this.log("Already Connected!");var d="CONNECT";a=this.option.connectionArgs=a||this.option.connectionArgs;var b=this.session.restore();this.session.freq.change(parseInt(this.session.freq.value)+1);if(!0==this.option.session){if("object"==typeof b)a=b,d="RESTORE";else if(!1==this.trigger("connect"))return!1;
...
APS.prototype.onMessage=function(a){try{a=JSON.parse(a)}catch(c){a=a.replace(/\\'/g,"'");try{a=JSON.parse(a)}catch(e){return e.type="close",this.trigger("dead",[e]),this.transport.close()}}var d,b,g=!1,j=!0,h;for(h in a)if(a.hasOwnProperty(h))switch(d=a[h].raw,b=a[h].data,this.log(">>>> ",d," <<<< ",b),d){case "LOGIN":j=!1;this.state=0==this.state?1:this.state;this.session.id=b.sessid;this.trigger("login",[b.sessid]);break;case "IDENT":var j=!1,g=!0,f=new APS.cUser(b.user,this);this.user=this.pipes[f.pubid]=
f;1==this.state&&this.trigger("ready");this.session.save();break;case "CHANNEL":d=new APS.channel(b.pipe,this);this.pipes[d.pubid]=d;this.channels[d.name]=d;if(b=b.users)for(h=0;h<b.length;h++)d.addUser(b[h]);b=d.name.toLowerCase();if("object"==typeof this.eQueue[b]){b=this.eQueue[b];var k;for(h in b)b.hasOwnProperty(h)&&(f=b[h][0],k=b[h][1],d.on(f,k))}d.trigger("joined",[this.user,d]);this.trigger("newChannel",[d]);break;case "SYNC":f=this.user;d=this.pipes[b.chanid];"message"==b.event&&(b.data=
decodeURIComponent(b.data));d instanceof APS.user?f.trigger(b.event,[b.data,f,d]):d.trigger(b.event,[b.data,f,d]);break;case "EVENT-X":d=this.pipes[b.pipe.pubid];d.trigger(b.event,[b.data,d,d]);this.option.autoUpdate&&d.update(b.pipe.properties);break;case "EVENT":f=this.pipes[b.from.pubid];"undefined"==typeof f&&b.from&&(client.pipe[b.from.pubid]=new APS.user(b.from,client),f=client.pipe[b.from.pubid]);d=this.pipes[b.pipe.pubid];d.pubid==f.pubid&&(d=this.user);"message"==b.event&&(b.data=decodeURIComponent(b.data));
d.trigger(b.event,[b.data,f,d]);this.option.autoUpdate&&(f.update(b.from.properties),d.pubid!=f.pubid&&d.update(b.pipe.properties));break;case "NOJOIN":this.trigger("notjoined",[b]);break;case "JOIN":d=this.pipes[b.pipe.pubid];f=d.addUser(b.user);d.trigger("join",[f,d]);this.option.autoUpdate&&d.update(b.pipe.properties);break;case "LEFT":d=this.pipes[b.pipe.pubid];f=this.pipes[b.user.pubid];delete d.users[b.user.pubid];this.option.autoUpdate&&d.update(b.pipe.properties);d.trigger("left",[f,d]);break;
case "SELFUPDATE":this.option.autoUpdate&&this.user.update(b.user);break;case "CLOSE":j=!1;break;case "ERR":j=!1;f=[b.code,b.value,b];switch(b.code){case "001":case "002":case "003":clearTimeout(this.poller);this.trigger("dead",f);break;case "004":case "250":this.state=0;this.session.destroy(!0);this.option.session&&this.reconnect();break;default:this.check()}this.trigger("error",f);this.trigger("error"+b.code,f);break;default:f=[];for(h in b)b.hasOwnProperty(h)&&f.push(b[h]);this.trigger("raw"+d,
f)}g&&1!=this.state&&(j=!0,this.state=1,!1!==this.trigger("restored")&&this.trigger("ready"));0==this.transport.id&&(j&&1==this.transport.state)&&this.check()};
APS.transport=function(a,c,e){this.state=0;this.stack=[];this.callback=c;var d=e.option.transport,b=Array.prototype.slice.call(arguments);if("object"==typeof d)for(var g in d){if(!1!=APS.transport[d[g]].apply(this,b))break}else"string"==typeof d&&APS.transport[d].apply(this,b);var j=new function(){if("XMLHttpRequest"in window)return XMLHttpRequest;if("ActiveXObject"in window){var b=["Msxml2.XMLHTTP.6.0","Msxml2.XMLHTTP.3.0","Msxml2.XMLHTTP","Microsoft.XMLHTTP"],a;for(a in b)try{return ActiveXObject(b[a])}catch(d){}}return!1};
this.request=function(b,a,d){var c=new j;c.addEventListener("load",function(){d(this.responseText)},!1);c.open("POST",b,!0);c.setRequestHeader("Content-type","application/x-www-form-urlencoded");c.send(a)};if(e.option.eventPush){var h=this.send.bind(this),f=function(b){c.onmessage(b)};this.send=function(b,a,d){if("Event"==d.cmd)return this.request(e.option.eventPush,"cmd="+b,f),"pushed";h.apply(this,[b,a])}}};
APS.transport.ws=APS.transport.wb=function(a,c,e){if("WebSocket"in window){this.id=6;this.loop=setInterval(e.check.bind(e,!0),4E4);var d=e.option.secure?"wss":"ws";try{var b=new WebSocket(d+"://"+a+"/6/")}catch(g){return c.onerror(g),!1}this.send=function(a,d){0<this.state?b.send(a):this.stack.push(a);"function"==typeof d&&d()}.bind(this);b.onerror=function(b){this.state=0;clearInterval(this.loop);c.onerror(b)}.bind(this);b.onopen=function(){this.state=2;for(var b=0;b<this.stack.length;b++)this.send(this.stack[b],
null,JSON.parse(this.stack[b])[0]);this.stack=[]}.bind(this);b.onmessage=function(b){c.onmessage(b.data)};b.onclose=function(b){clearInterval(this.loop);this.state=e.state=0;c.onerror(b)}.bind(this);this.close=function(){b.close();this.state=e.state=0}}else return e.log("No Websocket support"),!1};
APS.transport.lp=function(a,c,e){function d(b){b.origin==j+"://"+a&&b.source===g.contentWindow&&(this.state=1,this.callback.onmessage(b.data))}function b(){this.state=1;for(var b=0;b<this.stack.length;b++)this.send(this.stack[b],null,JSON.parse(this.stack[b])[0]);this.stack=[]}this.id=0;var g=document.createElement("iframe"),j=e.option.secure?"https":"http";c=window.location.protocol+"//"+window.location.host;a=a.toLowerCase();with(g.style)position="absolute",left=top="-10px",width=height="1px";document.body.appendChild(g);
g.setAttribute("src",j+"://"+a+'/?[{"cmd":"frame","params": {"origin":"'+c+'"}}]');"addEventListener"in window?(window.addEventListener("message",d.bind(this),0),g.addEventListener("load",b.bind(this),0)):window.attachEvent("onmessage",d.bind(this));this.send=function(b,d){0<this.state?(g.contentWindow.postMessage(b,j+"://"+a),this.state=2):this.stack.push(b);"function"==typeof d&&d()};this.close=function(){clearTimeout(e.poller);e.state=0}};
APS.user=function(a,c){Object.defineProperties(this,{update:{value:function(a){if(!a)return!1;a._rev=parseInt(a._rev);if(a._rev>this._rev)for(var b in a)this[b]!=a[b]&&(this[b]=a[b],c.trigger("user"+b+"Update",[a[b],this]),c.trigger("userUpdate",[b,a[b],this]))}},_rev:{value:null,configurable:!0,writable:!0},channels:{value:{},writable:!0},pubid:{value:a.pubid},pub:{value:c.pub.bind(c,a.pubid)},send:{value:c.send.bind(c,a.pubid)}});for(var e in a.properties)this[e]=a.properties[e]};
APS.cUser=function(a,c){Object.defineProperties(this,{update:{value:function(a){if(!a)return!1;a._rev=parseInt(a._rev);if(a._rev>this._rev)for(var b in a)this[b]!=a[b]&&(this[b]=a[b],this.trigger("user"+b+"Update",[a[b],this]),this.trigger("userUpdate",[b,a[b],this]))}},change:{value:function(a,b){if("object"==typeof a)var c=a;else c={},c[a]=b;this._client.sendCmd("propUpdate",c)}},_rev:{value:null,configurable:!0,writable:!0},_events:{value:{},writable:!0},_client:{value:c},pubid:{value:a.pubid},
channels:{value:{},writable:!0},on:{value:c.on.bind(this)},trigger:{value:c.trigger.bind(this)},log:{value:c.log.bind(c,"[CurrentUser]")}});for(var e in a.properties)this[e]=a.properties[e]};
